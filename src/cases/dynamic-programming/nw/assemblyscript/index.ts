...
export function main(s1: string, s2: string): string[] {
  ...
  for (i = 0; i < s1.length + 1; i++) {
    for (j = 0; j < s2.length + 1; j++) {
      ...
      if (i > 0 && j > 0) {
        if (newval == mat.get(i - 1).get(j) - G) direc.get(i).get(j).push(UP);
        if (newval == mat.get(i).get(j - 1) - G) direc.get(i).get(j).push(LEFT);
        if (newval == mat.get(i - 1).get(j - 1) + mat.get(i).get(j))
          direc.get(i).get(j).push(UL);
      } else {
        direc
          .get(i)
          .get(j)
  ...
  chars.push(new Array<string>());
  chars.push(new Array<string>());
  var I = s1.length;
  var J = s2.length;
  while (I > 0 || J > 0) {
    switch (direc.get(I).get(J).at(0)) {
      case UP:
        I--;
        chars.at(0).push(s1.charAt(I));
        chars.at(1).push('-');
        break;
      case LEFT:
        J--;
        chars.at(0).push('-');
        chars.at(1).push(s2.charAt(J));
        break;
      case UL:
        I--;
        J--;
