...
function lavamd(boxes1d: i32): void {
  ...
  for (i = 0; i < dim_cpu.boxes1d_arg; i++) {
    ...
    for (j = 0; j < dim_cpu.boxes1d_arg; j++) {
      ...
      for (k = 0; k < dim_cpu.boxes1d_arg; k++) {
        // current home box
        unchecked((box_cpu[nh].x = k));
        unchecked((box_cpu[nh].y = j));
        unchecked((box_cpu[nh].z = i));
        unchecked((box_cpu[nh].number = nh));
        unchecked((box_cpu[nh].offset = nh * NUMBER_PAR_PER_BOX));

        // initialize number of neighbor boxes
        unchecked((box_cpu[nh].nn = 0));
  ...
  for (i = 0; i < dim_cpu.space_elem; i = i + 1) {
    unchecked((rv_cpu[i].v = ((commonRandom() % 10) + 1) / 10.0)); // get a number in the range 0.1 - 1.0
    unchecked((rv_cpu[i].x = ((commonRandom() % 10) + 1) / 10.0)); // get a number in the range 0.1 - 1.0
    unchecked((rv_cpu[i].y = ((commonRandom() % 10) + 1) / 10.0)); // get a number in the range 0.1 - 1.0
    unchecked((rv_cpu[i].z = ((commonRandom() % 10) + 1) / 10.0)); // get a number in the range 0.1 - 1.0
  }

  // input (charge)
  qv_cpu = new StaticArray<f64>(dim_cpu.space_elem); // (fp*)malloc(dim_cpu.space_mem2);
  for (i = 0; i < dim_cpu.space_elem; i = i + 1) {
    unchecked((qv_cpu[i] = <f64>((commonRandom() % 10) + 1) / 10)); // get a number in the range 0.1 - 1.0
  }
...
function kernel_cpu(
  ...
  for (l = 0; l < dim.number_boxes; l = l + 1) {
    ...
    first_i = <i32>unchecked(box[l].offset);

    //  Do for the # of (home+neighbor) boxes
    for (k = 0; k < 1 + unchecked(box[l].nn); k++) {
      //  neighbor box - get pointer to the right box
      if (k == 0) {
        pointer = l; // set first box to be processed to home box
      } else {
        pointer = <i32>unchecked(box[l].nei[k - 1].number); // remaining boxes are neighbor boxes
      }

      first_j = <i32>unchecked(box[pointer].offset);
      ...
      for (i = 0; i < NUMBER_PAR_PER_BOX; i = i + 1) {
        for (j = 0; j < NUMBER_PAR_PER_BOX; j = j + 1) {
          ...
          vij = Math.exp(-u2);
          fs = 2 * vij;
          var dx = unchecked(rv[first_i + i].x - rv[first_j + j].x);
          var dy = unchecked(rv[first_i + i].y - rv[first_j + j].y);
          var dz = unchecked(rv[first_i + i].z - rv[first_j + j].z);
          fxij = fs * dx;
          fyij = fs * dy;
          fzij = fs * dz;

          // forces
          unchecked((fv[first_i + i].v += qv[first_j + j] * vij));
          unchecked((fv[first_i + i].x += qv[first_j + j] * fxij));
          unchecked((fv[first_i + i].y += qv[first_j + j] * fyij));
          unchecked((fv[first_i + i].z += qv[first_j + j] * fzij));
        }
      }
    }
  }
